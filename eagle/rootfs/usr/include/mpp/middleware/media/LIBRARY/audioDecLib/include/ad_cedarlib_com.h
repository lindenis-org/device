...
typedef struct __Ac320FileRead
{
    ...
    unsigned char   *bufStart;              //buffer�׵�ַ, abs buffer start address.
    int             BufToTalLen;            //�ܳ���, abs buffer total len.
	
    unsigned char   *bufReadingPtr;         //���ڶ���ָ��, abs buffer current rdPos
    int             BufLen;                 //��Ч��ݳ���, abs buffer valid data len.
	
    unsigned char   *bufWritingPtr;         //���ڶ���ָ��, abs buffer current wtPos.
    int             BufValideLen;           //���೤��, abs buffer left free len.
    //unsigned int    ReadingOffset;
    astream_fifo_t 	frmFifo;    //abs frame management.
    //add for android
