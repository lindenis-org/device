...
typedef struct CaptureTimeLapse
{
    ...
    int64_t capFrameIntervalUs;     //two frame's pts interval which we select to encode.
    int64_t videoFrameIntervalUs;   //the frame reconfig_pts interval after encode, 
    int64_t lastCapTimeUs;          //last encoded frame's pts,
    int64_t lastTimeStamp;          //we set last encoded frame's reconfig_pts. unit:us
} CaptureTimeLapse;
...
typedef struct VIDEOENCDATATYPE {
    ...
    struct list_head    mUsedOutFrameList; //VEncCompOutputBuffer, for non-tunnel mode. when mOutputPortTunnelFlag == FALSE, use it to store user occupied frames.
    int                 mFrameNodeNum;
...
ERRORTYPE VideoEncResetChannel(PARAM_IN COMP_HANDLETYPE hComponent);
